#!/usr/bin/env bash
# Install Finder Quick Action: Convert image to PNG (sips).
set -euo pipefail

REPO_ROOT="$(cd "$(dirname "$0")/.." && pwd)"
SCRIPT_PATH="${REPO_ROOT}/scripts/finder-convert-to-png.sh"
SERVICE_NAME="Convert to PNG.workflow"
...
BUNDLE_ID="com.sidekick.finderConvertPng"
MENU_TITLE="Convert to PNG"
...
echo "Installed: ${DEST}"
echo "Finder → Quick Actions → ${MENU_TITLE} (writes <name>.png beside the original)"
