 ...
 * Health Check
 *
 * Verifies that all Sidekick infrastructure is reachable:
...
async function timedCheck(name: string, fn: () => Promise<string | void>): Promise<Check> {
...
async function checkMemgraph(): Promise<string> {
  // Dynamic import to avoid loading neo4j-driver if not needed elsewhere
  const neo4j = await import("neo4j-driver");
  const driver = neo4j.default.driver(
    config.memgraphUri,
    config.memgraphUser ? neo4j.default.auth.basic(config.memgraphUser, config.memgraphPassword) : undefined,
  );
...
async function checkDocker(): Promise<string> {
  ...
  const services: Array<{ Name?: string; Service?: string; State?: string }> = [];
  for (const line of lines) {
    try { services.push(JSON.parse(line)); } catch { /* skip non-JSON lines */ }
  }
...
function checkApiKey(name: string, value: string): Check {
...
export async function checkHealth(): Promise<HealthResult> {
  ...
  let overall: HealthResult["overall"];
  if (infraFails >= 2) overall = "down";
...
async function main() {
  ...
  console.log(`\nSidekick Health: ${statusIcon[result.overall]} (${result.overall})`);
