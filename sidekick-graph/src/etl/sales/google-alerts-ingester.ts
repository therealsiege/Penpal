...
async function fetchAlertEmails(maxMessages = 50): Promise<string[]> {
  ...
  try {
    const { stdout } = await execAsync(
      `${GOG} gmail messages search 'in:"Google Alerts" newer_than:2d' --max ${maxMessages} --json --account fuzeelogik@gmail.com`,
      { timeout: 30000 },
    );
...
async function fetchMessageBody(messageId: string): Promise<{ subject: string; body: string } | null> {
  try {
    const { stdout } = await execAsync(
      `${GOG} gmail get ${messageId} --json --account fuzeelogik@gmail.com`,
      { timeout: 15000 },
    );
    ...
    if (msg.headers && typeof msg.headers === "object" && !Array.isArray(msg.headers)) {
      ...
      const subjectHeader = msg.headers.find(
    ...
    console.warn(`  Failed to fetch message ${messageId}:`, (err as Error).message);
    return null;
  }
}
...
function parseAlertEmail(subject: string, content: string): SourceArticle[] {
  ...
  if (!isHtml) {
    ...
    while ((urlMatch = urlPattern.exec(mainBlock)) !== null) {
      ...
      if (title.toLowerCase().includes("google alert")) continue;
    ...
    while ((match = linkPattern.exec(content)) !== null) {
      ...
      if (title.toLowerCase().includes("google alert") || title === "flag as irrelevant") continue;
      if (title.length < 10) continue;
    ...
    if (articles.length === 0) {
      ...
      while ((match = directPattern.exec(content)) !== null) {
        ...
        if (title.toLowerCase().includes("unsubscribe") || title.toLowerCase().includes("google")) continue;
...
export async function ingestAlerts(targetVentures?: VentureProfile[]): Promise<{
  ...
  const redditArticles: SourceArticle[] = redditPostsList.map((p) => ({
    title: p.title,
    url: p.url,
    source: "reddit.com",
  ...
  const hnArticles: SourceArticle[] = hnPostsList.map((p) => ({
    title: p.title,
    url: p.url,
  ...
  const ventureArticles = new Map<string, SourceArticle[]>();
  for (const article of allArticles) {
    const matchedVentures = routeAlertKeyword(article.keyword);
    for (const venture of matchedVentures) {
      if (!ventures.some((v) => v.slug === venture.slug)) continue;
