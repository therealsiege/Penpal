...
export const revenueOpportunitySchema = {
  ...
  description:
    "Calculate potential CMS revenue opportunity for a lead or practice based on specialty, EHR, and eligible programs (CCM, RPM, APCM, TCM, AWV, MIPS). Returns eligible programs with estimated annual revenue per patient and relevant billing codes.",
  inputSchema: {
    ...
    properties: {
      leadName: {
        type: "string",
        description:
          "Name of the lead to analyze. If omitted, use specialty to find opportunities.",
      },
      specialty: {
        type: "string",
        description:
          "Medical specialty to analyze (e.g. 'Family Medicine', 'Internal Medicine'). Used as fallback if leadName is not provided.",
      },
    },
  },
};
  ...
  try {
    ...
    if (args.leadName) {
      ...
      for (const record of leadResult.records) {
        ...
        const programs = record.get("programs") as Array<{
          program: string | null;
          fullName: string | null;
          annualRevPerPatient: number | null;
          payer: string | null;
        }>;
        ...
        if (eligiblePrograms.length > 0) {
          ...
          for (const p of eligiblePrograms) {
            const rev = typeof p.annualRevPerPatient === "object" && p.annualRevPerPatient !== null
              ? (p.annualRevPerPatient as unknown as { toNumber: () => number }).toNumber()
              : (p.annualRevPerPatient ?? 0);
            sections.push(
              `- **${p.program}** (${p.fullName}): $${rev.toFixed(2)}/patient/year [${p.payer}]`,
            );
            totalAnnual += rev;
          }
          sections.push("");
          sections.push(
            `**Estimated total per eligible patient**: $${totalAnnual.toFixed(2)}/year`,
          );
        } else {
          sections.push(
            "_No program eligibility computed. Run ETL to populate ELIGIBLE_FOR relationships._",
          );
        }
        sections.push("");
      }
    ...
    if (specialtyName) {
      ...
      const specResult = await session.run(
        `MATCH (p:Program)-[:ELIGIBLE_SPECIALTY]->(s:Specialty)
         ...
         ORDER BY p.annualRevenuePerPatient DESC`,
      ...
      if (specResult.records.length > 0) {
        ...
        for (const record of specResult.records) {
          ...
          const rev = typeof annualRev === "object" && annualRev !== null
            ? (annualRev as unknown as { toNumber: () => number }).toNumber()
            : (annualRev ?? 0);
          const codes = record.get("codes") as Array<{
            code: string | null;
          ...
          if (validCodes.length > 0) {
            ...
            for (const c of validCodes) {
              const codeRate = typeof c.rate === "object" && c.rate !== null
                ? (c.rate as unknown as { toNumber: () => number }).toNumber()
                : (c.rate ?? 0);
      ...
      const leadCount = await session.run(
        `MATCH (l:Lead)-[:ELIGIBLE_FOR]->(p:Program)
      ...
      if (leadCount.records.length > 0) {
        ...
        for (const record of leadCount.records) {
          ...
          const countVal = typeof count === "object" && count !== null
            ? (count as unknown as { toNumber: () => number }).toNumber()
            : count;
    ...
    const skillResult = await session.run(
      `MATCH (s:Skill)-[r:ENABLES_BILLING]->(bc:BillingCode)-[:PART_OF_PROGRAM]->(p:Program)
    ...
    if (skillResult.records.length > 0) {
      ...
      for (const record of skillResult.records) {
        sections.push(
          `| ${record.get("skill")} | ${record.get("status")} | ${record.get("role")} | ${(record.get("codes") as string[]).join(", ")} | ${(record.get("programs") as string[]).join(", ")} |`,
        );
      }
    }
