...
export const listCommunitiesSchema = {
  ...
  description:
    "Show entity clusters from community detection. Lists communities with their members, useful for discovering related entities and themes.",
  inputSchema: {
...
export async function listCommunities(args: {
  ...
  try {
    ...
    const output = result.records.map((record) => {
      ...
      const members = record.get("members") as {
        name: string;
        label: string;
      ...
      const memberList = sorted
        .map((m) => {
          const rank = m.pageRank ? ` (rank: ${(m.pageRank as number).toFixed(4)})` : "";
