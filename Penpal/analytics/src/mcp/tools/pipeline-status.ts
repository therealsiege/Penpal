...
export const pipelineStatusSchema = {
  ...
  description:
    "Sales pipeline analytics. Shows leads by stage, territory, EHR system, and scores. Can filter by various criteria.",
  inputSchema: {
  ...
  try {
    ...
    switch (groupBy) {
      ...
      case "score":
        ...
        `;
        if (args.minScore) params.minScore = args.minScore;
        if (args.territory) params.territory = args.territory;
        break;
    ...
    const rows = result.records.map((record) => {
      ...
      for (const key of keys) {
        ...
        if (val && typeof val === "object" && "toNumber" in val) {
          ...
          row[key as string] = Math.round(val * 100) / 100;
        } else {
