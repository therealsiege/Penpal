...
const PENPAL_DIR = path.resolve(__dirname, "..", "..", "..");

// Read presets from YAML to validate and suggest
const PRESETS_PATH = path.resolve(PENPAL_DIR, "agents", "agent-types.yaml");

export const podsCreateSchema = {
  name: "pods_create",
  description:
    "Create a new pod workflow. A pod is a 3-agent team (Solver/Reviewer/Executor) that implements, reviews, and tests a task with feedback loops. Optionally specify a preset team configuration.",
  inputSchema: {
    ...
    properties: {
      ...
      preset: {
        type: "string",
        description:
          'Team preset ID (e.g., "frontend-feature", "backend-feature", "full-stack", "content-pipeline"). Omit for default team.',
      },
      cwd: {
        type: "string",
        description:
          "Working directory for the pod. Defaults to the solver agent's default repo.",
      },
      priority: {
        type: "string",
        enum: ["critical", "high", "normal", "low"],
        description:
          'Task priority level. Controls compute allocation: critical gets best-of-3 sampling, normal gets single-shot. Defaults to "normal".',
      },
      candidates: {
        type: "number",
        description:
          "Number of solver candidates to generate (best-of-N sampling). Overrides the priority-based default. Higher values improve quality at the cost of more compute.",
      },
      maxSelfFixes: {
        type: "number",
        description:
          "Max self-fix attempts the executor can make before escalating to solver. Overrides the priority-based default. 0 = no self-fixes (immediate escalation).",
      },
    },
    required: ["task"],
  },
...
function loadPresets(): PresetInfo[] {
  try {
...
export async function podsCreate(args: {
  ...
  // Shell out to pod-cli.ts in the Penpal directory
  const cliScript = path.resolve(PENPAL_DIR, "src", "main", "pod-cli.ts");

  if (!fs.existsSync(cliScript)) {
    return JSON.stringify(
      {
        error: "Pod CLI script not found. Ensure Penpal/src/main/pod-cli.ts exists.",
        related_tools: ["pods_list", "pods_status"],
      },
      null,
      2,
  ...
  if (args.candidates && args.candidates > 0) cliArgs.push("--candidates", String(args.candidates));
  if (args.maxSelfFixes != null) cliArgs.push("--max-self-fixes", String(args.maxSelfFixes));
            ...
            JSON.stringify(
              {
                ...
                suggestions: [
                  "Check that the Penpal app dependencies are installed (cd Penpal && npm install)",
