...
export const vaultWriteSchema = {
  name: "vault_write",
  description:
    "Write or update a file in the Obsidian vault. Uses atomic writes (tmp + rename) for safety. Returns confirmation with updated backlinks.",
  inputSchema: {
...
export async function vaultWrite(args: {
  ...
  if (!exists && !args.createIfMissing) {
    throw new Error(
      `File not found: ${args.path}. Set createIfMissing: true to create it.`,
    );
  }
