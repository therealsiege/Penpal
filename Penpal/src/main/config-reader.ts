...
// Bundled agents/ dir resolution mirrors src/main/agents.ts:135-156.
// Dev: Penpal/agents/, packaged: app.asar/agents/.
...
function readMcpProfiles(): Record<string, McpServerEntry[]> {
  const profileDir = path.join(getBundledAgentsDir(), 'mcp-profiles')
...
function readClaudeSettings(): ClaudeSettingsSummary {
  ...
  const claudeMdPaths = [
    ...
    path.join(SIDEKICK_ROOT, 'Penpal', 'CLAUDE.md'),
    path.join(SIDEKICK_ROOT, 'Penpal', 'agents', 'CLAUDE.md'),
  ]
...
function readAgentToolSummaries(): AgentToolSummary[] {
  const agentYaml = path.join(getBundledAgentsDir(), 'agent-types.yaml')
  try {
...
export function addProfileMcpServer(
  ...
  const profilePath = path.join(SIDEKICK_ROOT, 'Penpal', 'agents', 'mcp-profiles', `${profileName}.json`)
  try {
...
export function updateAgentTools(
  ...
  const agentYaml = path.join(SIDEKICK_ROOT, 'Penpal', 'agents', 'agent-types.yaml')
  try {
    const yaml = require('js-yaml')
    const content = fs.readFileSync(agentYaml, 'utf-8')
