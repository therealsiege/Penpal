...
const SIDEKICK_GRAPH = path.resolve(__dirname, '..', '..', 'analytics')
...
export async function forceRunJob(jobName: string): Promise<JobRun> {
  if (app.isPackaged) {
    throw new Error('Scheduler force-run requires a developer checkout of Penpal/analytics. Not available in the packaged app.')
  }
  const jobs = loadJobs()
