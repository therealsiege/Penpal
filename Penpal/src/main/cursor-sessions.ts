...
function isCursorTopLevelCommand(command: string): boolean {
  const lower = command.toLowerCase()
  if (lower.includes('worker-server')) return false

  const launchedViaAgentShim =
    lower.includes('/.local/bin/agent') &&
    lower.includes('cursor-agent')
  const directRuntimeCommand =
    lower.includes('/.local/share/cursor-agent/') &&
    lower.includes('/index.js')

  return launchedViaAgentShim || directRuntimeCommand
}
...
async function findCursorAgentProcesses(): Promise<AgentProcess[]> {
  ...
  try {
    ...
    // truncated (which can hide cursor runtime signatures from matching).
    ...
    for (const rawLine of stdout.split('\n')) {
      ...
      const match = line.match(/^(\d+)\s+(\S+)\s+(\S+)\s+(\d+)\s+(.+)$/)
      if (!match) continue

      const pid = parseInt(match[1], 10)
      if (!Number.isFinite(pid) || pid <= 0) continue
      const command = match[5]
      if (!isCursorTopLevelCommand(command)) continue

      const cpuVal = parseFloat(match[3] || '0')
      const memoryKB = parseInt(match[4] || '0', 10)
      const cwd = await getWorkingDirectory(pid)
...
export async function getCursorAgentSessions(): Promise<CursorAgentSession[]> {
  ...
  for (const agent of agents) {
    const projectDir = findProjectDirByCwd(agent.cwd) || cwdToProjectDir(agent.cwd)
