...
function getPackagedAppPath(): string | null {
  try {
    return app?.isPackaged && typeof app.getAppPath === 'function' ? app.getAppPath() : null
  } catch {
    return null
  }
}
...
function getRootCandidates(): string[] {
  ...
  const candidates = [
    ...
    packagedAppPath,
    SIDEKICK_ROOT,
    path.join(SIDEKICK_ROOT, 'Penpal'),
    path.join(HOME_DIR, 'sidekick'),
    path.join(HOME_DIR, 'sidekick', 'Penpal'),
  ]
...
function resolveScanRoot(): { directory: string; source: SoundboardListing['source'] } {
  ...
  const defaultDirs = [
    ...
    path.join(SIDEKICK_ROOT, 'Penpal', 'sound_effects'),
    DEFAULT_SFX_DIR,
  ]
