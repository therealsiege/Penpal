    ...
    <div className="h-full flex flex-col overflow-hidden">
      <div className="shrink-0 px-2 py-2 border-b border-[color-mix(in_srgb,var(--c-border)_60%,transparent)]">
        <input
          ...
          disabled={!indexReady}
          className="w-full bg-[var(--c-bg-surface)] text-[var(--c-text-primary)] text-sm px-2 py-1.5 rounded border border-[color-mix(in_srgb,var(--c-border)_50%,transparent)] outline-none focus:border-blue-500 placeholder-[var(--c-border)]"
        />
        {query && results.length > 0 && (
          <div className="text-[10px] text-[var(--c-border)] mt-1">{results.length} results</div>
        )}
      </div>
      <div className="flex-1 overflow-y-auto">
        {results.map((r, i) => {
          ...
          return (
            ...
            >
              <div className="text-sm text-[var(--c-text-secondary)] truncate font-medium">{r.title}</div>
              <div className="text-[10px] text-[var(--c-border)] truncate">{folder}</div>
              {r.snippet && (
                <div className="text-[10px] text-[var(--c-border-hover)] truncate mt-0.5">{r.snippet}</div>
              )}
            </div>
          )
        })}
        {query && !searching && results.length === 0 && indexReady && (
          <div className="px-3 py-6 text-center text-[var(--c-border)] text-xs">No results</div>
        )}
        {searching && (
          <div className="px-3 py-6 text-center text-[var(--c-border)] text-xs animate-pulse">Searching...</div>
        )}
      </div>
    </div>
  )
