    ...
    <div className="h-full flex flex-col overflow-hidden">
      <div className="shrink-0 px-2 py-2 border-b border-slate-800/60">
        <input
          ...
          disabled={!indexReady}
          className="w-full bg-slate-900 text-slate-200 text-sm px-2 py-1.5 rounded border border-slate-700/50 outline-none focus:border-blue-500 placeholder-slate-600"
        />
      ...
      <div className="flex-1 overflow-y-auto">
        {results.map((r, i) => {
          ...
          return (
            ...
            >
              <div className="text-sm text-slate-300 truncate font-medium">{r.title}</div>
              <div className="text-[10px] text-slate-600 truncate">{folder}</div>
              {r.snippet && (
                <div className="text-[10px] text-slate-500 truncate mt-0.5">{r.snippet}</div>
              )}
            </div>
          )
        })}
