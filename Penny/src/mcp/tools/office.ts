...
export function buildOfficeAgentsResponse(snapshot: GameStateSnapshot): ContextEngineeredResponse<OfficeAgentsData> {
  ...
  if (idle > 0 && snapshot.orchestrator.queueDepth > 0) {
    suggestions.push(`${idle} idle agent(s) with ${snapshot.orchestrator.queueDepth} queued task(s) — assign via pod:create.`)
  }
  if (idle > 0 && snapshot.orchestrator.queueDepth === 0 && blocked === 0) {
    suggestions.push(`${idle} idle agent(s) — check orchestrator:queue for incoming work or pod:create to start workflows.`)
  }
