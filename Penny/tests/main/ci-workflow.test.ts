...
describe('CI workflow (.github/workflows/test.yml)', () => {
  ...
  it('sets working-directory to Penny', () => {
    expect(workflowContent).toContain('working-directory: Penny')
  })
  ...
  it('caches npm with Penny/package-lock.json', () => {
    expect(workflowContent).toContain("cache: 'npm'")
    expect(workflowContent).toContain('cache-dependency-path: Penny/package-lock.json')
  })
  ...
  it('uploads playwright-report artifact on failure', () => {
    ...
    expect(workflowContent).toContain('path: Penny/playwright-report/')
