...
Requires a sibling `sidekick-graph/` directory with a `.env` file containing Memgraph/Qdrant connection strings and optional Slack tokens.
...
| `scheduler-bridge.ts` | Reads `sidekick-graph/data/scheduler-state.json` and `schedule.yaml` for job statuses |
...
| `SettingsPanel.tsx` | Appearance and theme settings |
...
Per-project channels (`#sk-sidekick`, `#sk-medscrub`) via Socket Mode:
