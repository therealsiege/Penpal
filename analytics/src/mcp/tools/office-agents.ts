...
export const officeAgentsSchema = {
  name: "office_agents",
  description:
    "Get the current state of all agents in the Penny office: name, status (idle/active/sleeping/blocked), room assignment, current task, XP, rank, and credits. Includes a status breakdown and actionable suggestions.",
  inputSchema: {
...
export async function officeAgents(): Promise<string> {
  ...
  if (idleCount > 0) suggestions.push(`${idleCount} agent${idleCount > 1 ? "s" : ""} idle — consider dispatching tasks.`);
  if (blockedCount > 0) suggestions.push(`${blockedCount} agent${blockedCount > 1 ? "s" : ""} blocked — may need tool approval or intervention.`);
