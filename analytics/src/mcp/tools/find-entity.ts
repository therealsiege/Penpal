...
export const findEntitySchema = {
  name: "find_entity",
  description:
    "Look up a company, person, technology, or other entity in the knowledge graph. Returns the entity's properties and all its relationships with other entities.",
  inputSchema: {
...
export async function findEntity(args: {
  ...
  try {
    ...
    for (const record of findResult.records) {
      ...
      if (props.pageRank !== undefined) output += `\nPageRank: ${props.pageRank}`;
      if (props.communityId !== undefined) output += `\nCommunity: ${props.communityId}`;
      if (props.centrality !== undefined) output += `\nCentrality: ${props.centrality}`;
