...
export const searchLeadsSchema = {
  name: "search_leads",
  description:
    "Search leads by name, company, or location with optional filters for state, EHR system, and pipeline stage. Returns scored results with context-engineered metadata including scoring breakdown and suggested next actions.",
  inputSchema: {
...
function suggestAction(stage: string): string {
...
function scoringContext(lead: {
  ...
  const tier =
    lead.score >= 70 ? "High-scoring" : lead.score >= 40 ? "Mid-scoring" : "Early-stage";
  const parts = [`${tier} lead`];
  if (lead.stage) parts.push(`in ${lead.stage} stage`);
  if (lead.ehr) parts.push(`using ${lead.ehr}`);
  if (lead.businessArm) parts.push(`(${lead.businessArm})`);
...
export async function searchLeads(args: {
  query: string;
  filters?: { state?: string; ehr?: string; stage?: string };
}): Promise<string> {
