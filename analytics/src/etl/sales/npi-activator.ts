...
function parseFlags(): { limit: number; state: string | null; dryRun: boolean } {
...
function getExistingLeadNPIs(): Set<string> {
  const npis = new Set<string>();
  if (!fs.existsSync(LEADS_DIR)) return npis;
  ...
  for (const file of fs.readdirSync(LEADS_DIR)) {
    ...
    try {
      ...
      if (npiMatch) npis.add(npiMatch[1]);
    } catch { /* skip unreadable files */ }
  }
  return npis;
}

function getExistingLeadNames(): Set<string> {
  const names = new Set<string>();
  if (!fs.existsSync(LEADS_DIR)) return names;

  for (const file of fs.readdirSync(LEADS_DIR)) {
    if (!file.endsWith(".md")) continue;
    const nameMatch = file.match(/^(.+?)\s+[a-f0-9]{32}\.md$/);
    if (nameMatch) names.add(normalizeName(nameMatch[1]));
  }
  return names;
}
...
function scoreNPIProspect(p: NPIProspect): number {
  let score = 0;

  // Sole proprietor = likely independent (highest value for MedScrub)
  if (p.isSoleProprietor === "Y" || p.isSoleProprietor === "X") score += 20;

  // Priority specialty
  if (PRIORITY_SPECIALTIES.some((s) => p.specialty.includes(s))) score += 15;

  // Family Medicine > Internal Medicine > General Practice for MedScrub
  if (p.specialty.includes("Family Medicine")) score += 10;
  else if (p.specialty.includes("Internal Medicine")) score += 5;

  // Priority state
  if (PRIORITY_STATES.includes(p.state)) score += 10;
  ...
  if (p.enumerationDate) {
    ...
    if (currentYear - enumYear <= 5) score += 10;
    else if (currentYear - enumYear <= 10) score += 5;
  }

  return Math.min(score, 100);
}

function writeLead(p: NPIProspect, score: number): string {
  const id = generateId();
...
async function main() {
  ...
  try {
    ...
    const prospects: Array<NPIProspect & { score: number }> = [];
    ...
    for (const record of result.records) {
      ...
      // Dedup
      if (p.npi && existingNPIs.has(p.npi)) continue;
    ...
    if (!flags.dryRun) {
      ...
      if (activated.length > 10) console.log(`    ... and ${activated.length - 10} more`);
    }
