...
import type { ParsedLead } from "../parsers/lead-parser.js";
...
export async function extractCompanies(
  ...
  venture: VentureProfile,
): Promise<Map<string, SourceArticle[]>> {
...
If no qualifying companies found, return [].`,
  ...
  try {
    ...
    for (const entry of extracted) {
      if (!entry.company || entry.company.length < 2) continue;
...
export function getExistingLeadNames(venture: VentureProfile): Set<string> {
  const names = new Set<string>();
  const leadsDir = resolveVaultPath(venture.leadsDir);

  if (!fs.existsSync(leadsDir)) return names;
...
export async function enrichWithFirecrawl(companyName: string): Promise<EnrichmentResult> {
  ...
  try {
    ...
    const searchData = (await searchRes.json()) as {
      success: boolean;
      data?: Array<{ url: string; markdown?: string; metadata?: { description?: string } }>;
    };

    if (!searchData.success || !searchData.data?.length) return result;
    ...
    console.warn(`  Firecrawl enrichment failed for ${companyName}:`, (err as Error).message);
  }

  return result;
}

function extractSignals(text: string, result: EnrichmentResult): void {
...
export function writeLeadMarkdown(lead: EnrichedLead, leadSource: string): string {
  ...
  if (lead.fundingStage) bioLines.push(`Funding: ${lead.fundingStage}`);
  if (lead.employeeCount) bioLines.push(`~${lead.employeeCount} employees`);
  if (techStackStr) bioLines.push(`Tech signals: ${techStackStr}`);
...
export async function pushLeadToGraph(
...
export async function processCompanyForVenture(
  companyName: string,
  articles: SourceArticle[],
  venture: VentureProfile,
  importer: GraphImporter,
  ...
  try {
    ...
    const score = scoreLead(
      {
        ...
        emr: enrichment.techStack.some((t) =>
          ["Epic", "Cerner", "athenahealth"].includes(t),
        )
    ...
    console.warn(`  Failed to process ${companyName}:`, (err as Error).message);
    return false;
  }
}
