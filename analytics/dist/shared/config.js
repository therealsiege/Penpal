...
function resolveHome(p) {
    ...
    if (p.startsWith("$HOME/"))
        return path.join(homeDir, p.slice(6));
    return p;
}
...
export const config = {
    ...
    embeddingCachePath: path.join(homeDir, ".sidekick", "embedding-cache.json"),
    // LLM extraction cache location
    extractionCachePath: path.join(homeDir, ".sidekick", "extraction-cache.json"),
    // NPI Registry
    npiFilePath: process.env.NPI_FILE_PATH || path.join(defaultVaultPath, "Data", "NPPES", "npidata_pfile_20050523-20260308.csv"),
    npiCachePath: path.join(homeDir, ".sidekick", "npi-cache.json"),
...
export function getActiveVentures(filterKeys) {
...
export function resolveVenture(relPath) {
    for (const [key, v] of Object.entries(ventures)) {
        if (v.directories.some((d) => relPath.startsWith(d)))
            return key;
    }
    return "unknown";
}
