...
import { ensureCollections, upsertChunks, upsertSummaries, } from "./db/qdrant-client.js";
import { extractWithLLM, saveExtractionCache } from "./extraction/llm-extractor.js";
import { reconcileEntities, writeNewEntitiesReport } from "./extraction/entity-reconciler.js";
...
import { buildDocumentNode, buildFolderNode, buildTagNode, buildPersonNode, buildCompanyNode, buildTechnologyNode, buildEHRNode, buildSkillNode, buildRegulationNode, buildLeadNodeFromDoc, buildLeadNodeFromCRM, buildMarketNode, buildEventNode, buildBillingCodeNode, buildProgramNode, buildSpecialtyNode, buildPracticeNode, buildCompetitorProductNode, } from "./graph/node-builder.js";
import { buildInFolderRel, buildParentFolderRel, buildTaggedWithRel, buildLinksToRel, buildMentionRels, buildAboutLeadRel, buildWorksAtRels, buildCompetesWithRels, buildLeadUsesEHRRel, buildOperatesInRel, buildHadEventRel, buildReportedInRel, resolveEHRName, buildEnablesBillingRel, buildPartOfProgramRel, buildEligibleForRel, buildPracticesAtRel, buildInSpecialtyRel, buildEligibleSpecialtyRel, buildHasProductRel, } from "./graph/rel-builder.js";
...
async function main() {
    ...
    for (const v of activeVentures) {
        ...
        for (const d of v.directories)
            console.log(`    - ${d}`);
    }
    if (skipEmbeddings)
        console.log("  Skipping embeddings (--skip-embeddings)");
    if (skipLLM)
        console.log("  Skipping LLM extraction (--skip-llm)");
    if (skipAnalytics)
        console.log("  Skipping MAGE analytics (--skip-analytics)");
    if (skipNPI)
        console.log("  Skipping NPI enrichment (--skip-npi)");
    if (skipWiki)
        console.log("  Skipping wiki generation (--skip-wiki)");
    console.log();
    // 1. Connect
    ...
    for (const c of companies)
        importer.addNode(buildCompanyNode(c));
    for (const p of people)
        importer.addNode(buildPersonNode(p));
    for (const t of technologies)
        importer.addNode(buildTechnologyNode(t));
    for (const e of ehrSystems)
        importer.addNode(buildEHRNode(e));
    for (const r of regulations)
        importer.addNode(buildRegulationNode(r));
    for (const s of skills)
        importer.addNode(buildSkillNode(s));
    // Phase 4: Seed revenue intelligence nodes
    for (const bc of billingCodes)
        importer.addNode(buildBillingCodeNode(bc));
    for (const p of programs)
        importer.addNode(buildProgramNode(p));
    for (const sp of specialties)
        importer.addNode(buildSpecialtyNode(sp));
    ...
    for (const prog of programs) {
        for (const spec of prog.eligibleSpecialties) {
            const matchedSpec = specialties.find((s) => s.name.toLowerCase() === spec.toLowerCase());
            if (matchedSpec) {
    ...
    const docRelPaths = new Set(); // for link resolution later
    ...
    for (const relPath of mdFiles) {
        ...
        if (lead) {
            ...
            if (lead.company) {
                ...
                if (lead.type) {
                    const matchedSpec = specialties.find((s) => normalizeName(s.name) === normalizeName(lead.type)
        ...
        if (config.llmExtractionDocTypes.includes(doc.documentType)) {
            docsForLLM.push({
        ...
        if (fs.existsSync(crmPath)) {
            ...
            for (const crm of crmRecords) {
                ...
                if (crm.company) {
                    ...
                    if (crm.type) {
                        const matchedSpec = specialties.find((s) => normalizeName(s.name) === normalizeName(crm.type)
    ...
    if (!skipNPI) {
        ...
        if (config.npiFilePath) {
            try {
                ...
                console.warn(`  Warning: NPI CSV streaming failed:`, err.message);
            }
        }
    }
    else {
    ...
    if (webIntelRecords.length > 0) {
        for (const record of webIntelRecords) {
            ...
            const companyMatch = companies.find((c) => normalizeName(c.name) === normalizeName(record.company)
                || c.aliases?.some((a) => normalizeName(a) === normalizeName(record.company)));
            if (companyMatch) {
                importer.addRel(buildHasProductRel(companyMatch.name, record.product || record.company, record.company));
            }
        }
    ...
    if (!skipLLM && config.anthropicApiKey && docsForLLM.length > 0) {
        ...
        function processExtractionResult(doc, result) {
            // Reconcile entities
            ...
            for (const entity of reconciled) {
                if (entity.isNew) {
                    ...
                    if (entity.type === "company")
                        newEntitiesReport.companies.push(report);
                    else if (entity.type === "person")
                        newEntitiesReport.people.push(report);
                    else if (entity.type === "technology")
                        newEntitiesReport.technologies.push(report);
                }
            }
            // Process markets
...
function extractStateAbbr(location) {
    ...
    if (match) {
        const abbr = match[1];
        if (Object.values(STATE_ABBRS).includes(abbr))
            return abbr;
    }
    return null;
}
