 ...
 */
export async function streamNPIProspects() {
    ...
    const prospects = [];
    let rowCount = 0;
    const stream = createReadStream(npiFilePath).pipe(parse({ columns: true, quote: '"', relax_column_count: true }));
 ...
 */
export async function enrichLeadsFromNPI(leads) {
    const npiFilePath = config.npiFilePath;
    if (!npiFilePath) {
    ...
    for await (const row of stream) {
        ...
        if (matches.size >= leadIndex.size)
            break;
    }
 ...
 */
export async function queryNPIApi(params) {
    ...
    if (params.enumerationType)
        url.searchParams.set("enumeration_type", params.enumerationType);
 ...
 */
export async function batchQueryNPIApi(leads) {
