...
async function searchPrograms(page, stateId, specialtyId) {
    ...
    return page.$$eval("#programsListView-listview tbody tr", (trs) => trs
        .map((tr) => {
...
function parseDetailText(body, stateAbbr) {
...
async function scrapeDetail(page, href, stateAbbr) {
                ...
                for (let i = 0; i < listings.length; i++) {
                    ...
                    try {
                        ...
                        const linkExists = await page.evaluate((orgCode) => {
                            return !!Array.from(document.querySelectorAll('a[href*="Detail"]')).find(a => a.getAttribute('href')?.includes(orgCode));
                        }, listing.orgCode);
                        if (!linkExists)
                            throw new Error('link not found for ' + listing.orgCode);
                        ...
                        await Promise.all([
                            ...
                            page.evaluate((orgCode) => {
                                document.querySelectorAll('td, a').forEach(el => {
                                    el.style.cssText = 'display:revert!important;visibility:visible!important;';
                                });
                    ...
                    if (detail && detail.directorName) {
                        if (!detail.orgCode)
                            detail.orgCode = listing.orgCode;
                        if (!detail.programName)
                            detail.programName = listing.name;
                    ...
                    for (const orgCode of missing) {
                        ...
                        try {
                            ...
                            const linkExists = await page.evaluate((oc) => {
                                return !!Array.from(document.querySelectorAll('a[href*="Detail"]')).find(a => a.getAttribute('href')?.includes(oc));
                            }, orgCode);
                            if (!linkExists) {
                                await page.close();
                                continue;
                            ...
                            await Promise.all([
                                ...
                                page.evaluate((oc) => {
                                    ...
                                    const links = document.querySelectorAll('a[href*="Detail"]');
                                    for (const a of links) {
                                        if (a.getAttribute('href')?.includes(oc)) {
                                            a.click();
                                            return;
                                        }
                                    }
...
export function saveACGMEData(programs, outPath) {
...
if (process.argv[1]?.match(/acgme-scraper\.(ts|js)$/)) {
    ...
    for (let i = 0; i < process.argv.length; i++) {
        if (process.argv[i] === "--state" && process.argv[i + 1])
            states.push(process.argv[++i].toUpperCase());
        if (process.argv[i] === "--specialty" && process.argv[i + 1])
            specialties.push(process.argv[++i].toLowerCase());
    }
    ...
    scrapeACGME({
        ...
        .then((programs) => {
        if (!dryRun)
            saveACGMEData(programs, outPath);
    })
        .catch((err) => {
