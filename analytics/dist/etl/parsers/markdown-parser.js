...
import { normalizeTag } from "../../shared/utils/normalize.js";
import { config } from "../../shared/config.js";
/** Classify document type based on directory path */
function classifyDocType(relativePath) {
    const lower = relativePath.toLowerCase();
    if (lower.includes("sales/leads/"))
        return "lead";
    if (lower.includes("engineering/"))
        return "engineering";
    if (lower.includes("integrations/"))
        return "integration";
    if (lower.includes("advisors/"))
        return "advisor";
    if (lower.includes("competitors/"))
        return "competitor";
    if (lower.includes("product/skills/"))
        return "skill";
...
export function parseMarkdownFile(absolutePath) {
    ...
    // Parse dates
    const createdAt = parseDate(metadata["Created"]) || parseDate(metadata["CreatedTime"]) || null;
