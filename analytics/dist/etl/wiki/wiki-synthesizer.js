...
// ---------------------------------------------------------------------------
async function fetchRelevantChunks(query, limit = CHUNK_SEARCH_LIMIT) {
    try {
...
// ---------------------------------------------------------------------------
async function synthesize(systemPrompt, userContent, retries = 2) {
    ...
    for (let attempt = 0; attempt <= retries; attempt++) {
        try {
            ...
            const isRetryable = (err instanceof Error && err.message.includes("Overloaded")) ||
...
// Frontmatter helper
// ---------------------------------------------------------------------------
function frontmatter(type, name, tags) {
    return `---
type: ${type}
aliases: [${name}]
...
// Entity synthesizers
// ---------------------------------------------------------------------------
export async function synthesizeCompany(c) {
    ...
    const sourceContext = chunks.length > 0
        ...
        : "";
    const body = await synthesize(`You are writing a knowledge base page about a company for a healthcare startup founder.
...
export async function synthesizePerson(p) {
    ...
    const sourceContext = chunks.length > 0
        ? `\n\nSource document excerpts:\n${chunks.map((c, i) => `[${i + 1}] ${c}`).join("\n\n")}`
        : "";
    const body = await synthesize(`You are writing a knowledge base page about a person for a healthcare startup founder.
...
If a section has no useful content, omit it entirely.`, `Person graph data:\n${graphContext}${sourceContext}`);
    const tags = ["entity", "person"];
    return frontmatter("person", p.name, tags) + body;
}
export async function synthesizeTechnology(t) {
    ...
    const sourceContext = chunks.length > 0
        ? `\n\nSource document excerpts:\n${chunks.map((c, i) => `[${i + 1}] ${c}`).join("\n\n")}`
        : "";
    const body = await synthesize(`You are writing a knowledge base page about a technology/standard for a healthcare startup.
...
If a section has no useful content, omit it entirely. Be concise.`, `Technology graph data:\n${graphContext}${sourceContext}`);
    const tags = ["entity", "technology", t.category?.toLowerCase()].filter(Boolean);
    return frontmatter("technology", t.name, tags) + body;
}
export async function synthesizeLead(l) {
    ...
    const sourceContext = chunks.length > 0
        ? `\n\nSource document excerpts:\n${chunks.map((c, i) => `[${i + 1}] ${c}`).join("\n\n")}`
        : "";
    const body = await synthesize(`You are writing a knowledge base page about a sales lead for a healthcare startup.
...
If a section has no useful content, omit it entirely.`, `Lead graph data:\n${graphContext}${sourceContext}`);
...
export async function synthesizeProduct(p) {
    ...
    const sourceContext = chunks.length > 0
        ? `\n\nSource document excerpts:\n${chunks.map((c, i) => `[${i + 1}] ${c}`).join("\n\n")}`
        : "";
    const body = await synthesize(`You are writing a competitive intelligence page about a product for a healthcare startup founder.
Output pure markdown (no frontmatter). Start with H1, then:
...
If a section has no useful content, omit it entirely.`, `Product graph data:\n${graphContext}${sourceContext}`);
