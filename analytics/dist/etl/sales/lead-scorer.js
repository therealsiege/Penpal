...
export function scoreLead(input, profile = "clinical") {
    let score = 0;
    ...
    if (funnel.includes("closed won") || funnel.includes("customer"))
        score += 15;
    else if (funnel.includes("proposal") || funnel.includes("contract"))
        score += 12;
    else if (funnel.includes("demo"))
        score += 10;
    else if (funnel.includes("qualified"))
        score += 8;
    else if (funnel.includes("outreach") || funnel.includes("prospect"))
        score += 5;
    // Priority boost (up to 5 points)
    const priority = (input.priority || "").toLowerCase();
    if (priority.includes("high") || priority === "1")
        score += 5;
    else if (priority.includes("medium") || priority === "2")
        score += 3;
...
function scoreClinical(input, allText, titleAndType) {
    let score = 0;
...
function scoreIntegration(allText) {
    let score = 0;
