...
import { enrichWithFirecrawl } from "./shared-pipeline.js";
import { scoreLead } from "./lead-scorer.js";
import { ventureProfiles } from "./venture-config.js";
...
function parseLeadFile(filePath) {
    try {
...
// ─── Lead File Updater ──────────────────────────────────────────────────────
function updateLeadFile(lead, enrichment, newScore, detectedEHR) {
    ...
    for (let i = 0; i < newLines.length; i++) {
        ...
        if (line.startsWith("Bio:")) {
            ...
            if (enrichment.description)
                enrichBits.push(enrichment.description.slice(0, 300));
            if (enrichment.techStack.length > 0)
                enrichBits.push(`Tech: ${enrichment.techStack.join(", ")}`);
...
async function main() {
    ...
    for (const file of allFiles) {
        ...
        if (parsed.fields["Lead Source"] !== "NPI Registry")
            continue;
        if (parsed.fields["Next Action"] !== UNENRICHED_SENTINEL)
            continue;
        // Optional state filter
        if (flags.state) {
            const location = parsed.fields["Location"] || "";
            if (!location.includes(flags.state))
                continue;
        }
