...
import { getTargetVentures, routeArticleToVentures, } from "./venture-config.js";
...
function cleanItemLink(raw, feedUrl) {
...
async function fetchFeed(feedUrl, feedName) {
    const parser = new Parser({
        timeout: 15000,
        headers: {
            "User-Agent": "Penny-Analytics/1.0 (RSS Feed Reader)",
        },
    });
    async function attempt() {
        ...
        for (const item of feed.items || []) {
            const itemTitle = typeof item.title === "string" ? item.title : item.title?._ ?? JSON.stringify(item.title ?? "");
    ...
    try {
        ...
        console.warn(`  ${feedName}: fetch failed, retrying in 2s... (${err.message})`);
        await new Promise((r) => setTimeout(r, 2000));
        try {
            return await attempt();
        }
        catch (retryErr) {
            console.warn(`  ${feedName}: retry failed, skipping. (${retryErr.message})`);
            return [];
        }
    }
}
// ─── Main ────────────────────────────────────────────────────────────────────
export async function ingestRss(targetVentures) {
    ...
    const results = await Promise.allSettled(feeds.map((feed) => fetchFeed(feed.url, feed.name)));
    ...
    for (const venture of ventures) {
        ...
        const ventureArticles = [];
        for (const article of newArticles) {
            // Check if from a subscribed feed
            const isSubscribedFeed = [...feedMap.entries()].some(([url, feed]) => feed.name === article.feedName && ventureFeedUrls.has(url));
            if (isSubscribedFeed) {
