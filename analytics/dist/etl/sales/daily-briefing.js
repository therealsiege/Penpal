...
// ─── Penny state paths ────────────────────────────────────────────────────
const PENNY_DATA = path.resolve(os.homedir(), "sidekick", "Penny", "data");
...
// ─── Section Builders ───────────────────────────────────────────────────────
async function pipelineSummary(session) {
...
async function newLeads(session) {
    const result = await session.run(`MATCH (l:Lead)
     ...
     LIMIT 20`, { since: since24h });
    if (result.records.length === 0)
        return "## New Leads (24h)\n\nNo new leads in the last 24 hours.\n";
...
async function hotLeads(session) {
    const result = await session.run(`MATCH (l:Lead)
...
async function territoryBreakdown(session) {
    const result = await session.run(`
...
async function competitiveSignals(session) {
    const result = await session.run(`
...
async function actionItems(session) {
    const result = await session.run(`
    MATCH (l:Lead)
...
function dispatchWork() {
    ...
    const pipeline = readJsonSafe(PIPELINE_STATE, { issues: [] });
    ...
    // Pod workflows
    const pods = readJsonSafe(POD_STATE, []);
    ...
    // Task queue
    const queue = readJsonSafe(TASK_QUEUE, { tasks: [] });
    const recentTasks = (queue.tasks || []).filter(t => (t.updatedAt || t.createdAt || 0) > Date.now() - 24 * 60 * 60 * 1000);
...
function interestingLeads(session) {
    ...
    return session.run(`MATCH (l:Lead)
     ...
     RETURN l.name AS name, l.company AS company,
            ...
            e.name AS ehr
     ORDER BY l.leadScore DESC
     LIMIT 10`, { since: since12h }).then(result => {
        if (result.records.length === 0)
            return "## Notable Leads\n\nNo high-scoring leads from recent ingestion.\n";
...
// ─── Slack DM ──────────────────────────────────────────────────────────────
async function slackDmBriefing(briefingText, timeLabel) {
    ...
    try {
        ...
        console.warn(`  Slack DM failed: ${err.message}`);
    }
}
// ─── Main ───────────────────────────────────────────────────────────────────
async function main() {
    ...
    try {
        ...
        console.warn(`  Failed: Dispatch Activity — ${err.message}`);
    }
