...
app.get("/graph/neighbors/:id", async (c) => {
    ...
    try {
        // Get center node
        const centerResult = await session.run(`MATCH (n) WHERE id(n) = $nodeId RETURN n, labels(n)[0] AS type, id(n) AS nodeId`, { nodeId });
        ...
        const neighborsResult = await session.run(`MATCH (n)-[r]-(m)
       WHERE id(n) = $nodeId
