...
const POD_WORKFLOWS_PATH = path.resolve(__dirname, "..", "..", "..", "..", "Penny", "data", "pod-workflows.json");
const LEGACY_PATH = path.resolve(__dirname, "..", "..", "..", "..", "Penny", "data", "triplet-workflows.json");
export const podsStatusSchema = {
    name: "pods_status",
    description: "Get detailed status of a specific pod workflow including current phase, role statuses, stage history, artifacts, and completion estimates.",
    inputSchema: {
        type: "object",
        properties: {
            workflowId: {
...
const PHASE_DESCRIPTIONS = {
    ...
    paused: "Workflow is paused — can be resumed from the Penny dashboard",
};
...
function estimateCompletion(wf) {
    ...
    const totalStages = firstIterStages + (wf.maxIterations - 1) * subsequentIterStages;
...
export async function podsStatus(args) {
    ...
    const result = {
        ...
        suggestions: [
            ...(wf.status === "paused"
                ? ["Workflow is paused — resume it from the Penny dashboard"]
                : []),
