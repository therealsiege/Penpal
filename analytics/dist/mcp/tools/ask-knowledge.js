...
export const askKnowledgeSchema = {
    name: "ask_knowledge",
    description: "RAG-powered Q&A over the knowledge base. Ask a natural language question and get a synthesized answer with source citations. Uses semantic search + graph context + Claude Sonnet for answer generation.",
    inputSchema: {
        type: "object",
        properties: {
            question: {
...
export async function askKnowledge(args) {
    ...
    try {
        const docPaths = [
            ...new Set(results.map((r) => r.payload.documentPath)),
        ].slice(0, 5);
        for (const docPath of docPaths) {
         ...
         LIMIT 10`, { docPath });
            if (entityResult.records.length > 0) {
                const entities = entityResult.records.map((r) => `${r.get("label")}: ${r.get("name")}`);
    ...
    const response = await anthropic.messages.create({
        ...
        max_tokens: 2048,
        system: "You are a knowledgeable assistant for a healthcare technology company. Answer questions using ONLY the provided context from the knowledge base. Cite sources by document path. If the context doesn't contain enough information, say so clearly.",
        messages: [
            {
                role: "user",
    ...
    // Add sources
    const sources = [
        ...new Set(results.slice(0, 5).map((r) => r.payload.documentPath)),
    ];
