...
export const updateLeadSchema = {
    name: "update_lead",
    description: "Update a lead in the sales pipeline. Can change stage, priority, notes, or next action. Updates both the vault markdown file and Memgraph.",
    inputSchema: {
        type: "object",
        properties: {
            name: {
...
export async function updateLead(args) {
    ...
    try {
        ...
        if (args.stage) {
            ...
            await session.run(`MATCH (l:Lead {id: $leadId})-[r:CURRENT_STAGE]->(:SalesStage)
         DELETE r`, { leadId });
            await session.run(`MATCH (l:Lead {id: $leadId}), (s:SalesStage {id: $stageId})
         MERGE (l)-[:CURRENT_STAGE]->(s)`, { leadId, stageId });
        }
        if (args.priority) {
        ...
        if (setClause.length > 0) {
            await session.run(`MATCH (l:Lead {id: $leadId})
         SET ${setClause.join(", ")}`, params);
            updates.push("Memgraph updated");
        }
    }
    catch (err) {
        updates.push(`Memgraph error: ${err.message}`);
    }
    finally {
        await session.close();
    }
