...
export const vaultSearchSchema = {
    name: "vault_search",
    description: "Search the Obsidian vault for files matching a query. Returns ranked results with snippets, tag context, and folder hierarchy.",
    inputSchema: {
        type: "object",
        properties: {
            query: {
...
export async function vaultSearch(args) {
    ...
    if (!sanitized)
        throw new Error("Query contains no searchable characters");
...
function grepSearch(query, limit) {
    return new Promise((resolve) => {
        ...
        execFile("grep", args, { maxBuffer: 2 * 1024 * 1024, timeout: 15000 }, (err, stdout) => {
            if (err && !stdout) {
                resolve([]);
                return;
            }
