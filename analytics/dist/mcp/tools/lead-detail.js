...
export const leadDetailSchema = {
    name: "lead_detail",
    description: "Get full lead profile including relationships, event history, related documents, stage history, and suggested next actions based on current pipeline stage.",
    inputSchema: {
        type: "object",
        properties: {
            leadId: {
...
function suggestedNextActions(stage) {
...
export async function leadDetail(args) {
    ...
    try {
       ...
       RETURN l, s.name AS stage, e.name AS ehr,
              t.name AS territory, c.name AS companyNode`, { name: args.leadId });
        ...
        const [eventsResult, docsResult, historyResult] = await Promise.all([
            session.run(`MATCH (l:Lead {name: $name})-[:HAD_EVENT]->(ev:Event)
         RETURN ev.type AS type, ev.date AS date, ev.detail AS detail
         ORDER BY ev.date DESC LIMIT 20`, { name: args.leadId }),
            session.run(`MATCH (l:Lead {name: $name})<-[:ABOUT_LEAD]-(d:Document)
         RETURN d.title AS title, d.path AS path
         ORDER BY d.title LIMIT 20`, { name: args.leadId }),
            session.run(`MATCH (l:Lead {name: $name})-[:ENTERED_STAGE]->(s:SalesStage)
         RETURN s.name AS stage, s.enteredAt AS enteredAt
         ORDER BY s.enteredAt DESC`, { name: args.leadId }),
        ]);
