...
const { parseFrontmatter, findBacklinks, extractTags } = await import("../vault-helpers.js");
...
`;
    ...
    fs.writeFileSync(path.join(tmpDir, "subfolder", "deep-note.md"), "# Deep Note\n\nA note in a subfolder about #integration patterns.\n");
});
after(() => {
...
describe("vault_read", () => {
    ...
    it("includes backlinks from other files", async () => {
        ...
        const backlink = result.backlinks.find((b) => b.title === "related-note");
    ...
    it("includes related files from same folder", async () => {
        ...
        assert.ok(result.relatedFiles.some((f) => f.includes("related-note.md")));
    });
...
describe("vault_search", () => {
    it("finds files matching a query", async () => {
        ...
        const match = result.find((r) => r.path.includes("test-note.md"));
    ...
    it("returns empty for no matches", async () => {
        const result = JSON.parse(await vaultSearch({ query: "zzzznonexistent12345" }));
        assert.ok(Array.isArray(result));
        assert.equal(result.length, 0);
    });
    it("respects limit parameter", async () => {
        const result = JSON.parse(await vaultSearch({ query: "note", limit: 1 }));
...
describe("vault_write", () => {
    it("creates a new file with createIfMissing", async () => {
        ...
        const result = JSON.parse(await vaultWrite({
            path: "new-note.md",
            content,
    ...
    it("creates nested directories with createIfMissing", async () => {
        ...
        assert.equal(result.success, true);
        // Verify file exists
        const readResult = JSON.parse(await vaultRead({ path: "new-folder/sub/deep-new.md" }));
        assert.equal(readResult.content, content);
    });
