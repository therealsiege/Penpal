...
export const getPipelineSchema = {
    name: "get_pipeline",
    description: "Full CRM pipeline view. Returns leads grouped by pipeline stage with scores, next actions, and company info. Can filter by stage, business arm, or minimum score.",
    inputSchema: {
        type: "object",
        properties: {
            stage: {
...
export async function getPipeline(args) {
    ...
    try {
        ...
        const result = await session.run(`MATCH (s:SalesStage)
       ...
       ORDER BY stageOrder, score DESC`, params);
        // Group by stage
        const stageMap = new Map();
        for (const record of result.records) {
            const stageName = record.get("stage");
            ...
            if (name) {
                ...
                stage.leads.push({
                    ...
                    businessArm: record.get("businessArm") || "",
                });
                stage.count++;
            }
        }
