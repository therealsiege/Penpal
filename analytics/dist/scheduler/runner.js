...
function parseYaml(text) {
    ...
    for (const rawLine of text.split("\n")) {
        ...
        if (jobMatch) {
            ...
            if (currentJob && currentDef.command) {
                if (dependsList.length > 0)
                    currentDef.depends_on = [...dependsList];
                jobs[currentJob] = currentDef;
            }
        ...
        if (propMatch && currentJob) {
            ...
            switch (key) {
                case "description":
                    currentDef.description = val;
                    break;
                case "command":
                    currentDef.command = val;
                    break;
                case "cron":
                    currentDef.cron = val;
                    break;
                case "timeout":
                    currentDef.timeout = parseInt(val, 10);
                    break;
                case "enabled":
                    currentDef.enabled = val === "true";
                    break;
                case "depends_on":
                    inDependsOn = true;
                    break;
...
// ─── Job Execution ─────────────────────────────────────────────────────────
export function executeJob(jobName, job) {
...
function dependenciesMet(job, state, now) {
...
export function getJobHistory(jobName, limit = 20) {
    const state = loadState();
    const runs = jobName
...
export function forceRunJob(jobName) {
    const schedule = loadSchedule();
...
export function tick() {
    ...
    for (const [name, job] of Object.entries(schedule.jobs)) {
        if (!job.enabled)
            continue;
        if (!isDue(job.cron, now, state.last_run[name]))
            continue;
