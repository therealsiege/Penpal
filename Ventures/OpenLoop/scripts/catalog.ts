...
export const organizations: OrganizationDef[] = [
  ...
  // Added from codebase enrichment (AI & LLM Integration)
  { name: "OpenAI", type: "vendor", description: "LLM provider — GPT-4 integration via Medplum $ai operation" },
];
...
export const technologies: TechnologyDef[] = [
  ...
  // Added from codebase enrichment (Bots & Subscriptions, AI, React Components)
  { name: "BullMQ", category: "framework" },
  { name: "FHIRcast", category: "protocol" },
  { name: "MCP", category: "protocol" },
  { name: "OpenAI GPT-4", category: "tool" },
  { name: "AWS Textract", category: "cloud" },
  { name: "AWS Comprehend Medical", category: "cloud" },
  { name: "Medplum React", category: "framework" },
  { name: "Medplum CLI", category: "framework" },
  { name: "pdfmake", category: "tool" },
  { name: "QuestionnaireForm", category: "framework" },
  { name: "SearchControl", category: "framework" },
  { name: "PlanDefinitionBuilder", category: "framework" },
];
...
export const integrations: IntegrationDef[] = [
  ...
  // Added from codebase enrichment (AI & LLM Integration)
  { name: "OpenAI $ai Operation", vendor: "OpenAI", category: "clinical" },
  { name: "Medplum MCP Server", vendor: "Medplum", category: "clinical" },
  { name: "AWS Textract OCR", vendor: "AWS", category: "clinical" },
  { name: "AWS Comprehend Medical NLP", vendor: "AWS", category: "clinical" },
];
...
export const fhirResources: FhirResourceDef[] = [
  ...
  // Added from codebase enrichment (Server Operations, Care Plans, AI docs)
  { name: "RequestGroup", description: "Ordered group of actions — generated by PlanDefinition $apply" },
  { name: "ActivityDefinition", description: "Reusable action template — referenced by PlanDefinition actions" },
  { name: "DetectedIssue", description: "Drug interaction alerts and clinical decision support flags" },
  { name: "Medication", description: "Drug reference — RxNorm code, form, ingredients" },
  { name: "CoverageEligibilityRequest", description: "Request to verify insurance eligibility with payer" },
  { name: "CoverageEligibilityResponse", description: "Payer response — eligible services, copay, deductible" },
  { name: "PaymentReconciliation", description: "Reconcile payments against submitted claims" },
  { name: "ClaimResponse", description: "Payer's initial response to claim submission" },
  { name: "Measure", description: "Clinical quality measure definition (CQL-based)" },
  { name: "MeasureReport", description: "Results of evaluating a Measure against patient data" },
  { name: "ValueSet", description: "Set of codes from one or more CodeSystems — used for validation" },
  { name: "CodeSystem", description: "Code system definition (SNOMED, LOINC, ICD-10, RxNorm, custom)" },
  { name: "ConceptMap", description: "Maps codes between systems — critical for Healthie→FHIR migration" },
  { name: "StructureDefinition", description: "FHIR profile definition — constrains resource shapes" },
  { name: "OperationDefinition", description: "Custom FHIR operation definition — points to Bot for execution" },
  { name: "AccessPolicy", description: "Medplum-specific: fine-grained RBAC with criteria-based filtering" },
  { name: "ProjectMembership", description: "Medplum-specific: links User to Project with role and AccessPolicy" },
  { name: "AsyncJob", description: "Tracks long-running operations (bulk export, migrations)" },
  { name: "VerificationResult", description: "Identity verification outcome — IAL2 proofing results" },
];
...
export const decisions: DecisionDef[] = [
  ...
  // Added from codebase enrichment (Care Plans & Tasks, Server Operations, AI)
  { id: "D17", title: "$extract for intake data extraction over custom bots", rationale: "Native FHIR SDC operation auto-generates Observation/Condition from QuestionnaireResponse — no bot code needed for standard vitals/BMI", project: "MedPlum Migration" },
  { id: "D18", title: "Custom FHIR operations via OperationDefinition+Bot for Partners API", rationale: "Define OperationDefinition pointing to Bot — enables custom endpoints (e.g., $assign-provider) without server modification, hides FHIR from non-healthcare clients", project: "MedPlum Migration" },
  { id: "D19", title: "Project $clone for client onboarding automation", rationale: "Clone a template Project with standard AccessPolicies, Bots, Subscriptions for each new client — automates 120+ tenant setup", project: "MedPlum Migration" },
  { id: "D20", title: "WebSocket subscriptions for real-time provider dashboard", rationale: "Redis-backed WebSocket subscriptions for live UI updates without job queue overhead — ideal for encounter status, task assignments", project: "MedPlum Migration" },
  { id: "D21", title: "Event chain pattern (bots triggering bots) for workflow orchestration", rationale: "Each bot does one thing, communicates via FHIR resources, each link has independent retry — replaces monolithic workflow handlers", project: "MedPlum Migration" },
];
...
export const techMappings: TechMappingDef[] = [
  ...
  // Added from codebase enrichment
  { from: "SQS", to: "BullMQ" },
  { from: "GraphQL", to: "FHIR R4" },
  { from: "Step Functions", to: "PlanDefinition" },
];
